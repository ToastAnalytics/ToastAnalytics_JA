...
このドキュメントは、AndroidでAnalytics SDKを連動するための方法について説明します。Analytics SDKを使うためには、まずアプリを登録する必要があります。アプリの登録方法は、リンク（<http://cloud.toast.com/docudocum/2/>）をご参照ください。
...
1. Analytics SDKのダウンロード
    <http://cloud.toast.com/documents/6/>　からAndroid SDKファイルをダウンロードします。
...
（Proguardを使うためには、以下のオプションを追加する必要があります。
「-keep public class com.toast.android.analytics.external.leaktest.MemoryInfo { public *;}」）

![Project Setting](https://raw.githubusercontent.com/ToastAnalytics/ToastAnalytics_JA/master/docs/Developer/images/pg_aos_001.png)
...
<!—外部ストレージの書き込み権限。送信せず、内部で使うデータキャッシュのために使います。-->
...
ストア情報は、アプリをダウンロードしたマーケットに対する情報を利用者が追跡するための設定です。「Google」、「TStore」、「Naver」などマーケットに合わせて変更して使います。マーケットの文字列は、ゲームで自由に定義できます。
...
SDKを使うためには、アプリ登録後に発行される「アプリID」と「カンパニーID」が必要です。アプリの登録方法は、リンク（<http://cloud.toast.com/documents/2/>）をご参照ください。

initializeSDK関数のAppIDは、アプリ情報の「AppKey」を、CompanyIDは「カンパニーID」を使います。

![App Key]（https://raw.githubusercontent.com/ToastAnalytics/ToastAnalytics_JA/master/docs/Developer/images/pg_aos_002.png）
...
もし、初期化関数の最後の引数（use logging userid flag）をtrueに設定した場合、setLoggingUserIdを呼び出し、User IDを登録する必要があります。Flagをtrueに設定し、setLoggingUserIdを呼び出さない場合、それ以降呼び出す全てのAPIが失敗（E_LOGGING_USER_ID_EMPTY）をReturnします。
...
###プッシュの連動
Analytics SDKではキャンペーンのために、GCMを使います。GCMを使うためには、Analytics利用者ページにProject NumberとAPI Keyを登録する必要があります。さらに、SDKにはSenderIDを登録する必要があります。

GCMに関する全般的な内容は、Googleが提供する「Google Cloud Message」ドキュメントをご参照ください。
...
サーバーでプッシュ（GCM）メッセージを受信すると、Analytics SDKはNotification Barに通知を表示します。この通知をタッチすれば、アプリが立ち上がります。ここで、何も設定しなければ、そのアプリのMain Activityが表示されます

Main Activity以外のActivityを起動するためには、「setPushIntent」関数を通じて明示的に指定する必要があります。
...
また、 GCM プッシュを受け取って処理するロジック(レシーバー、サービス、Notification)も利用者が直接実装しなければなりません。
...
プッシュpayloadを構成する際、「cid」フィールドは、利用者定義プッシュにおいて使ってはなりません。「cid」フィールドは、SDKが提供する基本プッシュで内部的に使うタグとして、その主な使用目的はキャンペーン情報の送信です。
...
ここから送られた文字列を参考し、ゲーム内の動作を処理します。（今後リリース予定）
...
##SDKバージョン確認
SDKバージョンは、「&lduo;getVersion()」関数を通じて確認できます。
